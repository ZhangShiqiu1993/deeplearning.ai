...
def cosine_similarity(u, v):
    ...
    norm_v = np.sqrt(np.sum(np.square(v))) # np.linalg.norm(v)
...
def equalize(pair, bias_axis, word_to_vec_map):
    ...
    mu = (e_w1 + e_w2) / 2 # np.mean([e_w1, e_w2], axis=0)
    ...
    corrected_e_w1B = np.sqrt(np.abs(1 - np.sum(np.square(mu_orth)))) * ((e_w1B - mu_B) / np.abs((e_w1 - mu_orth) - mu_B))
    corrected_e_w2B = np.sqrt(np.abs(1 - np.sum(np.square(mu_orth)))) * ((e_w2B - mu_B) / np.abs((e_w2 - mu_orth) - mu_B))
